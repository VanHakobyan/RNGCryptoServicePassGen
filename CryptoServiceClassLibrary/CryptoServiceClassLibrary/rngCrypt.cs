...
namespace CryptoServiceClassLibrary
{
    public class rngCrypt
    {
        ...
        public class Password
        {
            ...
            public Password NewPassword(optionPass option = optionPass.pasDigitUpperSmall, int length = 32)
            {
                using (var rng = new RNGCryptoServiceProvider())
                {
                    ...
                    for (var i = 0; i < passChars.Length; i++)
                    {
                        ...
                        if (option == optionPass.pasSize) sb.Append(lower[rnd.Next(0, 62)]);
                        if (option == optionPass.pasUpperOnly) sb.Append(upper[rnd.Next(0, 36)]);
                        if (option == optionPass.pasSmallOnly) sb.Append(symbols[rnd.Next(0, 36)]);
                        if (option == optionPass.pasDigitOnly) sb.Append(numbers[rnd.Next(0, 52)]);
            ...
            {
                getPassword = NewPassword(optionPass.pasDigitUpperSmall, length).ToString();
            }
